...
- Changed files:
    ...
    - `src/app/hooks/usePrevious.ts` — retyped to `useRef<T | undefined>(undefined)` returning `T | undefined`, removing the `any` cast and matching the actual runtime where `ref.current` is `undefined` before the first `useEffect` flush.
    - `src/app/hooks/useFocus.ts` — widened the optional `inputRef` param to `React.RefObject<HTMLInputElement | null>` (React 19 `useRef(null)` return type) and accepted `previousListLength: number | undefined` to match the new `usePrevious` return type, with an explicit `undefined` guard before the comparison.
    - `src/app/søknader/korrigeringAvInntektsmelding/components/Periodepanel.tsx` — added an explicit `previousListLength !== undefined` guard around the inline `previousListLength < currentListLength` comparison to match the tightened `usePrevious` return type.
    - `src/app/components/calendar/TidsbrukKalender.tsx` — introduced an exported `ModalContentProps` type (`selectedDates?`, `toggleModal?`, `clearSelectedDates?`) and typed `ModalContent` as `React.ReactElement<ModalContentProps>`, with `React.cloneElement<ModalContentProps>(...)` at the call site. Removes the previous `any` and the scoped `eslint-disable`, and gives consumers proper feedback when modal-content props drift.
    - `src/app/components/calendar/TidsbrukKalenderÅr.tsx` and `src/app/components/calendar/TidsbrukKalenderContainer.tsx` — forwarded the new `ModalContentProps` type so the wrappers tighten in lockstep.
    - `src/app/components/calendar/KalenderMedModal.tsx` — typed `tidModal` as `React.ReactElement<ModalContentProps>` so the public API exposes the same contract.
    - `copilot-tasks/react-19-upgrade.md` — `Plan`, `Progress notes`, and `Outcome` updated.
