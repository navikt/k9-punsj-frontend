...
- Changed files:
    ...
    - `src/app/hooks/usePrevious.ts` — passed an explicit `undefined` initial value to `useRef<T>(undefined)` for the React 19 `useRef` overload tightening.
    ...
    - `src/app/components/calendar/TidsbrukKalender.tsx` — typed `ModalContent` as `React.ReactElement<any>` so `React.cloneElement(ModalContent, { selectedDates, ... })` still type-checks under React 19's stricter `cloneElement` inference. Local lint disable scoped to the single line.
