...
- Review `resolutions` against the declared dependencies of every updated direct package before finalizing the lockfile. Pay particular attention to exact dependency pins: when `react-intl` changes, compare its FormatJS requirements with the existing `@formatjs/*` and `intl-messageformat` overrides. Update or remove incompatible overrides and verify their lockfile entries together. Use `yarn why` for affected paths; leave unrelated overrides alone.
