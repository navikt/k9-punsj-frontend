...
jest.mock('app/containers/pleiepenger/FagsakReadMode', () => ({
  FagsakReadMode: () => <></>,
}));
jest.mock('app/containers/pleiepenger/SoknadReadMode', () => ({
  SoknadReadMode: () => <></>,
}));
jest.mock('app/utils/browserUtils', () => ({
  getHash: () => '#/hentsoknader/testident',
  setHash: jest.fn(),
}));
...
describe('MapperOgFagsaker', () => {
  ...
  it('Velger en ufullstendig søknad', () => {
    ...
    mapperOgFagsaker
      .find('ModalWrapper .punch_mappemodal_knapperad .knapp1')
      .simulate('click');
    expect(chooseMappeAction).toHaveBeenCalledTimes(1);
    expect(chooseMappeAction).toHaveBeenCalledWith(
      expect.objectContaining({ mappeId: mappe.mappeId })
    );
  });
});
