...
const setupIdent = (
  ...
  const identComponentProps: IIdentComponentProps = {
    identInput: (disabled: boolean) => (
      <Input value={fnr} {...{ disabled }} label="" />
    ),
...
describe('Fordeling', () => {
  /* it('Viser radioknapper for signatur', () => {
    ...
    expect(ident.find('RadioPanelGruppe').prop('legend')).toEqual(
      'ident.signatur.etikett'
    );
  });
...
*/
  it('Viser suksessmelding når usignert søknad er sendt videre', () => {
    ...
    expect(ident.find('AlertStripeSuksess').children().text()).toEqual(
      'ident.usignert.sendt'
    );
  });
  ...
  it('Viser feilmelding når det har oppstått en feil i videresending av usignert søknad', () => {
    ...
    const ident = setupIdent({
      ...
      usignertRequestError: { status: 404 },
    });
    expect(ident.find('AlertStripeFeil')).toHaveLength(1);
    expect(ident.find('AlertStripeFeil').children().text()).toEqual(
      'ident.usignert.feil.melding'
    );
  });
