...
describe('PunchPage', () => {
    it('Laster inn oversikt over ufullstendige søknader', () => {
        ...
        expect(punchPage.find('RegistreringsValg').prop('journalpostid')).toEqual(journalpostid);
    });
    ...
    it('Laster inn søknadsskjema', () => {
        ...
        const ident1 = '12345678901';
        const mappeid = 'abc';
        const punchPage = setupPunchPage(journalpostid, `#/hentsoknad/${mappeid}`, { step: 1, ident1 }, mappeid);
        expect(punchPage.find('PSBPunchForm')).toHaveLength(1);
        expect(punchPage.find('PSBPunchForm').prop('journalpostid')).toEqual(journalpostid);
