...
export function sjekkOmSkalTilK9SakFlereBarn(
    norskIdent: string,
    jpid: string,
    fagsakYtelseType: FagsakYtelseType,
    barn: string[]
    ...
    return (dispatch: any) => {
        ...
        const promises = barn.map((identifikator) =>
            post(
                ...
                res.json().then((skalTilK9Sak) => ({ response: res, k9sak: skalTilK9Sak.k9sak, barn: identifikator }))
            )
        );
        ...
        Promise.all(promises).then((responseList) => {
            if (responseList.every((res) => res.response.ok)) {
                ...
                return dispatch(sjekkSkalTilK9SuccessAction(kanIkkeGaaTilK9.length === 0));
            }
