...
interface IAuthOkAction         {type: AuthActionKeys.OK,       name: string}
...
const authOkAction      = (name: string):           IAuthOkAction       => ({type: AuthActionKeys.OK, name});
...
export function checkAuth() {return (dispatch: any) => {
    ...
    fetch(URL_AUTH_CHECK, {credentials: 'include'}).then(response => {
        switch (response.status) {
            case 200:   return response.json().then(user => dispatch(authOkAction(user.name)));
            case 401:   return dispatch(redirectAction(loginUrl()));
            default:    return dispatch(authErrorAction(convertResponseToError(response)));
        }
    });
}}