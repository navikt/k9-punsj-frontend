...
interface IUsignertErrorAction      {type: SignaturActionKeys.USIGNERT_ERROR, error: IError}
interface IUsignertResetAction      {type: SignaturActionKeys.USIGNERT_RESET}

type UsignertActionTypes = IUsignertRequestAction | IUsignertSuccessAction | IUsignertErrorAction | IUsignertResetAction;
...
const           usignertErrorAction     = (error: IError):          IUsignertErrorAction    => ({type: SignaturActionKeys.USIGNERT_ERROR, error});
export const    usignertResetAction     = ():                       IUsignertResetAction    => ({type: SignaturActionKeys.USIGNERT_RESET});

export const usignert = (journalpostid: string) => {return (dispatch: any) => {
    ...
    post(
        ...
        response => {
            if (response.status === 204) {return dispatch(usignertSuccessAction())}
