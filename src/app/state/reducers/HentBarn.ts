...
export interface IHentBarnForbiddenAction {
    type: ActiontypesHentBarn.HENTBARN_FORBIDDEN;
}
export interface IHentBarnSuccessAction {
    type: ActiontypesHentBarn.HENTBARN_SUCCESS;
    barn: IBarn[];
}
export interface IHentBarnRequestAction {
    type: ActiontypesHentBarn.HENTBARN_REQUEST;
}
export interface IHentBarnErrorAction {
    type: ActiontypesHentBarn.HENTBARN_ERROR;
}
    ...
    return (dispatch: any) => {
        ...
        return get(ApiPath.BARN_GET, { norskIdent }, { 'X-Nav-NorskIdent': norskIdent }, (response, barn) => {
            switch (response.status) {
                case 200:
                    return dispatch(getHentBarnSuccessAction(barn.barn));
                case 403:
                    return dispatch(getHentBarnForbiddenAction());
                default:
                    return dispatch(getHentBarnErrorAction());
            }
        });
    };
}
