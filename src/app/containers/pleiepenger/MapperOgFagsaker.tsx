...
export const MapperOgFagsakerComponent: React.FunctionComponent<IMapperOgFagsakerProps> = (
  props: IMapperOgFagsakerProps
) => {
  const {
    intl,
    punchState,
  ...
  React.useEffect(() => {
    if (
      ...
      mapperOgFagsakerState.isMappeCreated
    ) {
      setHash(
        getPunchPath(PunchStep.FILL_FORM, { id: mapperOgFagsakerState.mappeid })
      );
  ...
  if (
    mapperOgFagsakerState.mapperRequestError ||
    mapperOgFagsakerState.fagsakerRequestError
  ) {
    return (
      <>
        <AlertStripeFeil>
  ...
  const technicalError =
    mapperOgFagsakerState.isMappeCreated && !mapperOgFagsakerState.mappeid ? (
    ...
    for (const fagsak of fagsaker) {
      ...
      rows.push(
        <tr key={fagsak_id} onClick={() => props.openFagsakAction(fagsak)}>
          ...
          <td>
            {barn?.foedselsdato &&
              datetime(intl, TimeFormat.DATE_SHORT, barn.foedselsdato)}
          </td>
          <td>
            {fra_og_med && datetime(intl, TimeFormat.DATE_SHORT, fra_og_med)}
          </td>
          <td>
            {til_og_med && datetime(intl, TimeFormat.DATE_SHORT, til_og_med)}
          </td>
        </tr>
      );
      modaler.push(
        ...
        >
          <div className="modal_content">
            ...
            <div className="punch_mappemodal_knapperad">
              ...
              <Knapp className="knapp2" onClick={props.closeFagsakAction}>
                Lukk
              </Knapp>
            </div>
          </div>
  ...
  const newSoknadButton = (
    <p>
      <Knapp onClick={newMappe}>Opprett ny søknad</Knapp>
    </p>
  );
  ...
  if (fagsaker.length && !mapper.length) {
    return (
      <>
        ...
        {technicalError}
        <AlertStripeInfo>
          Det finnes fagsaker knyttet til identitetsnummeret. Velg saken som
          hører til dokumentet eller opprett en ny søknad.
        </AlertStripeInfo>
        {newSoknadButton}
        {showFagsaker()}
      </>
  ...
  if (mapper.length && fagsaker.length) {
    return (
      <>
        ...
        {technicalError}
        <AlertStripeInfo>
          Det finnes ufullstendige søknader og fagsaker knyttet til
          identitetsnummeret. Velg søknaden eller fagsaken som hører til
          dokumentet eller opprett en ny søknad.
        </AlertStripeInfo>
        {showMapper()}
        {newSoknadButton}
        {showFagsaker()}
...
const mapStateToProps = (
  ...
  mapperOgFagsakerState: state.PLEIEPENGER_SYKT_BARN.mapperOgFagsakerState,
});
...
const mapDispatchToProps = (dispatch: any) => ({
  ...
  findMapper: (ident1: string, ident2: string | null) =>
    dispatch(findMapper(ident1, ident2)),
...
export const MapperOgFagsaker = injectIntl(
  connect(mapStateToProps, mapDispatchToProps)(MapperOgFagsakerComponent)
);
